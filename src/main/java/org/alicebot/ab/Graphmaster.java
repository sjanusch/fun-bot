...
public class Graphmaster {
  ...
  void addSets(String type, Bot bot, Nodemapper node, String filename) {
    ...
    if (bot.getSetMap().containsKey(setName)) {
      ...
      System.out.println("No AIML Set found for <set>" + setName + "</set> in " + filename);
    }
  }
  ...
  private String printgraph(Nodemapper node, String partial) {
    StringBuilder stringBuilder = new StringBuilder();
    if (node == null) {
      stringBuilder.append("Null graph\n");
    } else {
      String template = "";
      if (NodemapperOperator.isLeaf(node) || node.shortCut) {
        ...
        if (node.shortCut) {
          stringBuilder.append(partial + "(" + NodemapperOperator.size(node) + "[" + node.height + "])--<THAT>-->X(1)--*-->X(1)--<TOPIC>-->X(1)--*-->" + template + "...\n");
        } else {
          stringBuilder.append(partial + "(" + NodemapperOperator.size(node) + "[" + node.height + "]) " + template + "...\n");
        }
      }
      for (String key : NodemapperOperator.keySet(node)) {
        stringBuilder.append(printgraph(NodemapperOperator.get(node, key), partial + "(" + NodemapperOperator.size(node) + "[" + node.height + "])--" + key + "-->\n"));
      }
    }
  ...
  public HashSet<String> getVocabulary() {
    ...
    for (String set : bot.getSetMap().keySet()) vocabulary.addAll(bot.getSetMap().get(set));
    return vocabulary;
  }
